...
namespace BrokenVector.PersistentComponents.Utils
{
    public static class SearchUtils
    {
        ...
        private static void RefreshStyles()
        {
            ...
            toolbar = skin != null ? skin.FindStyle("Toolbar") : null;
            search = skin != null ? skin.FindStyle("ToolbarSearchTextField") : null;
            if (search == null && skin != null)
                search = skin.FindStyle("ToolbarSeachTextField");

            //button = skin.FindStyle("ToolbarButton");
            cancelButton = skin != null ? skin.FindStyle("ToolbarSearchCancelButton") : null;
            if (cancelButton == null && skin != null)
        ...
        {
            if (search == null || cancelButton == null)
                RefreshStyles();

            GUILayout.BeginHorizontal(toolbar ?? GUIStyle.none);

            searchText = GUILayout.TextField(searchText ?? string.Empty, search ?? GUI.skin.textField);
            if (GUILayout.Button("", cancelButton ?? GUI.skin.button) || Event.current.keyCode == KeyCode.Escape)
            {
                searchText = "";
